...
training partition and dynamic GPU augmentation only for that partition:
right-angle rotations plus horizontal and vertical flips. These operations
preserve the `256 x 256 x 3` input shape and avoid interpolation during the
grid search.
